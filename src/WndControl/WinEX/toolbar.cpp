...
#define TOOLBARCLASSNAMEW       L"MyToolbarWindow32"
#define TOOLBARCLASSNAMEA        "MyToolbarWindow32"

static HCURSOR hCursorDrag = NULL;

static int toolbar = 2;
...
static LRESULT _LButtonDown(TOOLBAR_INFO *infoPtr, WPARAM wParam, LPARAM lParam);
static void TOOLBAR_LayoutToolbar(TOOLBAR_INFO *infoPtr);
static LRESULT _AutoSize(TOOLBAR_INFO *infoPtr);
...
static LRESULT _SetButtonInfo(TOOLBAR_INFO *infoPtr, INT Id, const TBBUTTONINFOW *lptbbi, BOOL isW);
...
static void TOOLBAR_DumpTBButton(const TBBUTTON *tbb, BOOL fUnicode)
{
    TRACE("TBBUTTON: id %d, bitmap=%d, state=%02x, style=%02x, data=%p, stringid=%p (%s)\n", tbb->idCommand,
        ...
        tbb->iString != -1 ? (fUnicode ?  (void *)debugstr_w((LPWSTR)tbb->iString) :  (void *)debugstr_a((LPSTR)tbb->iString)) :  (void *)"");
}
...
static inline BOOL TOOLBAR_ButtonHasString(const TBUTTON_INFO *btnPtr)
{
...
static inline BOOL TOOLBAR_IsValidImageList(const TOOLBAR_INFO *infoPtr, INT index)
{
 ...
 *      an image list
 */
static void TOOLBAR_DrawString (const TOOLBAR_INFO *infoPtr, RECT *rcText, LPCWSTR lpText,
...
static void TOOLBAR_DrawImage(const TOOLBAR_INFO *infoPtr, TBUTTON_INFO *btnPtr, INT left, INT top,
                  const NMTBCUSTOMDRAW *tbcd, DWORD dwItemCDFlag)
{
...
static void TOOLBAR_DrawButton (const TOOLBAR_INFO *infoPtr, TBUTTON_INFO *btnPtr, HDC hdc, DWORD dwBaseCustDraw)
{
    ...
    if (btnPtr->fsStyle & BTNS_SEP) {
        ...
        if ((dwStyle & TBSTYLE_FLAT) /* && (btnPtr->iBitmap == 0) */) {
	        ...
	        else TOOLBAR_DrawFlatSeparator (&rc, hdc, infoPtr);
	    }
...
*/
static inline SIZE TOOLBAR_MeasureButton(const TOOLBAR_INFO *infoPtr, SIZE sizeString, BOOL bHasBitmap, BOOL bValidImageList)
{
    SIZE sizeButton;
...
static void TOOLBAR_Cust_InsertAvailButton(HWND hwnd, PCUSTOMBUTTON btnInfoNew)
{
    ...
    TRACE("button %s, idCommand %d hwndhwnd %ld\n", debugstr_w(btnInfoNew->text), btnInfoNew->btn.idCommand, hwndAvail);
    ...
    //    {
    //        i = SendMessageW(hwndAvail, LB_INSERTSTRING, i, (LPARAM)L"");
    //        SendMessageW(hwndAvail, LB_SETITEMDATA, i, (LPARAM)btnInfoNew);
    //        return;
    //    }
    //}
    /* id higher than all others add to end */
    i = SendMessageW(hwndAvail, LB_ADDSTRING, 0, (LPARAM)L"");
    SendMessageW(hwndAvail, LB_SETITEMDATA, i, (LPARAM)btnInfoNew);
}
...
static void TOOLBAR_Cust_AddButton(const CUSTDLG_INFO *custInfo, HWND hwnd, INT nIndexAvail, INT nIndexTo)
{
    ...
    if (TOOLBAR_SendNotify(&nmtb.hdr, custInfo->tbInfo, TBN_QUERYINSERT))
    {
        ...
        else
        {
            ...
            btnNew = new CUSTOMBUTTON{0};//(CUSTOMBUTTON*)Alloc(sizeof(CUSTOMBUTTON));
            *btnNew = *btnInfo;
            btnInfo = btnNew;
        }
    ...
    switch (uMsg)
    {
	    case WM_INITDIALOG:
	        ...
	        if (custInfo)
	        {
		        ...
		        if (TOOLBAR_SendNotify (&nmtbic.hdr, infoPtr, TBN_INITCUSTOMIZE) == TBNRF_HIDEHELP)
                {
                ...
                {
                    btnInfo = new CUSTOMBUTTON{0};// = (CUSTOMBUTTON*)Alloc(sizeof(CUSTOMBUTTON));
		            ...
		            index = (int)SendDlgItemMessageW (hwnd, IDC_TOOLBARBTN_LBOX, LB_ADDSTRING, 0, (LPARAM)L"");
		        ...
		        for (i = 0;; i++)
		        {
		            ...
		            TRACE("WM_INITDIALOG style: %x iItem(%d) idCommand(%d) iString(%ld) %s\n",
                                ...
                                nmtb.tbButton.iString > 0 ? (void*)debugstr_w(infoPtr->strings[nmtb.tbButton.iString])
                                : "");
                    ...
                    { // sanity check
                        ...
                        {
                            if (*nmtb.pszText) lstrcpyW(btnInfo->text, nmtb.pszText);
                            else if (nmtb.tbButton.iString > 0 
                                && nmtb.tbButton.iString < infoPtr->nNumStrings)
                            {
                                lstrcpyW(btnInfo->text, infoPtr->strings[nmtb.tbButton.iString]);
                            }

                            if (index==-1) TOOLBAR_Cust_InsertAvailButton(hwnd, btnInfo);
                        }
                    }
		        }
...
static BOOL TOOLBAR_AddBitmapToImageList(TOOLBAR_INFO *infoPtr, HIMAGELIST himlDef, const TBITMAP_INFO *bitmap)
{
    ...
    if (bitmap->hInst == NULL)         /* a handle was passed */
        ...
        hbmLoad = (HBITMAP)LoadImageW( bitmap->hInst, MAKEINTRESOURCEW(bitmap->nID),
 ...
 *
 */
static LRESULT _AddBitmap (TOOLBAR_INFO *infoPtr, INT count, const TBADDBITMAP *lpAddBmp)
{
    TBITMAP_INFO info;
    INT iSumButtons, i;
    HIMAGELIST himlDef;
    ...
    { /* create new default image list */
        ...
        himlDef = ImageList_Create (infoPtr->nBitmapWidth, infoPtr->nBitmapHeight, ILC_COLOR32 | ILC_MASK, info.nButtons, 2);
...
static LRESULT _AddButtonsT(TOOLBAR_INFO *infoPtr, INT nAddButtons, const TBBUTTON* lpTbb, BOOL fUnicode)
{
...
static LRESULT _AddStringW (TOOLBAR_INFO *infoPtr, HINSTANCE hInstance, LPARAM lParam)
{
#define MAX_RESOURCE_STRING_LENGTH 512
    ...
    if (IS_INTRESOURCE(lParam)) 
    {
        ...
        len = LoadStringW (hInstance, (UINT)lParam, szString, MAX_RESOURCE_STRING_LENGTH);
        ...
        while ((next_delim = wcschr(p, delimiter)) != NULL) 
        {
            *next_delim = 0;
            ...
            infoPtr->strings = (LPWSTR*)ReAlloc(infoPtr->strings, sizeof(LPWSTR)*(infoPtr->nNumStrings+1));
...
static LRESULT _AddStringA (TOOLBAR_INFO *infoPtr, HINSTANCE hInstance, LPARAM lParam)
{
    BOOL fFirstString = (infoPtr->nNumStrings == 0);
    LPSTR p;
    INT nIndex;
    ...
    if (IS_INTRESOURCE(lParam))  /* load from resources */
        return _AddStringW(infoPtr, hInstance, lParam);
...
static inline LRESULT _ButtonCount (const TOOLBAR_INFO *infoPtr)
{
    return infoPtr->nNumButtons;
}

static inline LRESULT _ButtonStructSize (TOOLBAR_INFO *infoPtr, DWORD Size)
{
...
static LRESULT _ChangeBitmap (TOOLBAR_INFO *infoPtr, INT Id, INT Index)
{
    TBUTTON_INFO *btnPtr;
    INT nIndex;
...
static LRESULT _CheckButton (TOOLBAR_INFO *infoPtr, INT Id, LPARAM lParam)
{
    TBUTTON_INFO *btnPtr;
    INT nIndex;
    INT nOldIndex = -1;
    ...
    else 
    {
        ...
        {
	        nOldIndex = TOOLBAR_GetCheckedGroupButtonIndex (infoPtr, nIndex);
	        if (nOldIndex == nIndex)
		        return 0;
	        if (nOldIndex != -1)
...
static LRESULT _CommandToIndex (const TOOLBAR_INFO *infoPtr, INT Id)
{
    return TOOLBAR_GetButtonIndex (infoPtr, Id, FALSE);
}

static LRESULT _Customize (TOOLBAR_INFO *infoPtr)
{
...
static LRESULT _DeleteButton (TOOLBAR_INFO *infoPtr, INT nIndex)
{
    NMTOOLBARW nmtb;
    ...
    else 
    {
        ...
        if (nIndex > 0) {
            memcpy (&infoPtr->buttons[0], &oldButtons[0], nIndex * sizeof(TBUTTON_INFO));
        }

        if (nIndex < infoPtr->nNumButtons) {
            memcpy (&infoPtr->buttons[nIndex], &oldButtons[nIndex+1], (infoPtr->nNumButtons - nIndex) * sizeof(TBUTTON_INFO));
        }
...
static LRESULT _EnableButton (TOOLBAR_INFO *infoPtr, INT Id, LPARAM lParam)
{
    TBUTTON_INFO *btnPtr;
    INT nIndex;
    DWORD bState;
...
static inline LRESULT _GetAnchorHighlight (const TOOLBAR_INFO *infoPtr)
{
    return infoPtr->bAnchor;
}

static LRESULT _GetBitmap (const TOOLBAR_INFO *infoPtr, INT Id)
{
    INT nIndex;
...
static inline LRESULT _GetBitmapFlags (void)
{
    return (GetDeviceCaps (0, LOGPIXELSX) >= 120) ? TBBF_LARGE : 0;
}

static LRESULT _GetButton (const TOOLBAR_INFO *infoPtr, INT nIndex, TBBUTTON *lpTbb)
{
    TBUTTON_INFO *btnPtr;
...
static LRESULT _GetButtonInfoT(const TOOLBAR_INFO *infoPtr, INT Id, LPTBBUTTONINFOW lpTbInfo, BOOL bUnicode)
{
    ...
    if (lpTbInfo->dwMask & TBIF_TEXT) {
        ...
        if (!IS_INTRESOURCE(btnPtr->iString) && (btnPtr->iString != -1)) 
        {
            ...
            if (bUnicode)
                StringCchCopy(lpText, lpTbInfo->cchText, lpTbInfo->pszText);
            else
            {}//fixme
...
static inline LRESULT _GetButtonSize (const TOOLBAR_INFO *infoPtr)
{
    return MAKELONG((WORD)infoPtr->nButtonWidth,
                    (WORD)infoPtr->nButtonHeight);
}

static LRESULT _GetButtonText (const TOOLBAR_INFO *infoPtr, INT Id, LPWSTR lpStr, BOOL isW)
{
    INT nIndex;
    LPWSTR lpText;
    LRESULT ret = 0;
...
static LRESULT _GetDisabledImageList (const TOOLBAR_INFO *infoPtr, WPARAM wParam)
{
...
static inline LRESULT _GetExtendedStyle (const TOOLBAR_INFO *infoPtr)
{
    TRACE("\n");
...
static LRESULT _GetHotImageList (const TOOLBAR_INFO *infoPtr, WPARAM wParam)
{
    TRACE("hwnd=%p, wParam=%ld\n", infoPtr->hwndSelf, wParam);
    /* UNDOCUMENTED: wParam is actually the ID of the image list to return */
...
static LRESULT _GetHotItem (const TOOLBAR_INFO *infoPtr)
{
...
static LRESULT _GetDefImageList (const TOOLBAR_INFO *infoPtr, WPARAM wParam)
{
    TRACE("hwnd=%p, wParam=%ld\n", infoPtr->hwndSelf, wParam);
    /* UNDOCUMENTED: wParam is actually the ID of the image list to return */
...
static LRESULT _GetInsertMark (const TOOLBAR_INFO *infoPtr, TBINSERTMARK *lptbim)
{
...
static inline LRESULT _GetInsertMarkColor (const TOOLBAR_INFO *infoPtr)
{
    TRACE("hwnd = %p\n", infoPtr->hwndSelf);
...
static LRESULT _GetItemRect (const TOOLBAR_INFO *infoPtr, INT nIndex, LPRECT lpRect)
{
    TBUTTON_INFO *btnPtr;
...
static LRESULT _GetMaxSize (const TOOLBAR_INFO *infoPtr, LPSIZE lpSize)
{
    if (lpSize == NULL)
	    return FALSE;
...
static inline LRESULT _GetPadding (const TOOLBAR_INFO *infoPtr)
{
    return MAKELONG(infoPtr->szPadding.cx, infoPtr->szPadding.cy);
}

static LRESULT _GetRect (const TOOLBAR_INFO *infoPtr, INT Id, LPRECT lpRect)
{
    TBUTTON_INFO *btnPtr;
    INT        nIndex;
...
static inline LRESULT _GetRows (const TOOLBAR_INFO *infoPtr)
{
    return infoPtr->nRows;
}

static LRESULT _GetState (const TOOLBAR_INFO *infoPtr, INT Id)
{
    INT nIndex;
...
static inline LRESULT _GetStyle (const TOOLBAR_INFO *infoPtr)
{
    return infoPtr->dwStyle;
}

static inline LRESULT _GetTextRows (const TOOLBAR_INFO *infoPtr)
{
    return infoPtr->nMaxTextRows;
}

static LRESULT _GetToolTips (TOOLBAR_INFO *infoPtr)
{
...
static LRESULT _GetUnicodeFormat (const TOOLBAR_INFO *infoPtr)
{
...
static inline LRESULT _GetVersion (const TOOLBAR_INFO *infoPtr)
{
    return infoPtr->iVersion;
}

static LRESULT _HideButton (TOOLBAR_INFO *infoPtr, INT Id, BOOL fHide)
{
    TBUTTON_INFO *btnPtr;
    BYTE oldState;
    INT nIndex;
...
static inline LRESULT _HitTest (const TOOLBAR_INFO *infoPtr, const POINT* lpPt)
{
    return TOOLBAR_InternalHitTest (infoPtr, lpPt, NULL);
}

static LRESULT _Indeterminate (const TOOLBAR_INFO *infoPtr, INT Id, BOOL fIndeterminate)
{
    TBUTTON_INFO *btnPtr;
    INT nIndex;
    DWORD oldState;
...
static LRESULT _InsertButtonT(TOOLBAR_INFO *infoPtr, INT nIndex, const TBBUTTON *lpTbb, BOOL fUnicode)
{
    if (lpTbb == NULL)
	return FALSE;
...
static LRESULT _IsButtonChecked (const TOOLBAR_INFO *infoPtr, INT Id)
{
    INT nIndex;
...
static LRESULT _IsButtonEnabled (const TOOLBAR_INFO *infoPtr, INT Id)
{
    INT nIndex;
...
static LRESULT _IsButtonHidden (const TOOLBAR_INFO *infoPtr, INT Id)
{
    INT nIndex;
...
static LRESULT _IsButtonHighlighted (const TOOLBAR_INFO *infoPtr, INT Id)
{
    INT nIndex;
...
static LRESULT _IsButtonIndeterminate (const TOOLBAR_INFO *infoPtr, INT Id)
{
    INT nIndex;
...
static LRESULT _IsButtonPressed (const TOOLBAR_INFO *infoPtr, INT Id)
{
    INT nIndex;
...
static LRESULT _LoadImages (TOOLBAR_INFO *infoPtr, WPARAM wParam, HINSTANCE hInstance)
{
    TBADDBITMAP tbab;
    ...
    return _AddBitmap(infoPtr, 0, &tbab);
}

static LRESULT _MapAccelerator (const TOOLBAR_INFO *infoPtr, WCHAR wAccel, UINT *pIDButton)
{
    ...
    for (i = 0; i < infoPtr->nNumButtons; i++)
    {
            ...
            && !(btnPtr->fsState & TBSTATE_HIDDEN))
        {
            ...
            while (*lpszStr)
            {
                ...
                if (!_wcsnicmp(lpszStr, wszAccel, iLen))
                {
...
static LRESULT _MarkButton (const TOOLBAR_INFO *infoPtr, INT Id, BOOL fMark)
{
    INT nIndex;
    DWORD oldState;
    TBUTTON_INFO *btnPtr;
...
static LRESULT _MoveButton (TOOLBAR_INFO *infoPtr, INT Id, INT nMoveIndex)
{
    INT nIndex;
    INT nCount;
    TBUTTON_INFO button;
...
static LRESULT _PressButton (const TOOLBAR_INFO *infoPtr, INT Id, BOOL fPress)
{
    TBUTTON_INFO *btnPtr;
    INT nIndex;
    DWORD oldState;
...
static LRESULT _ReplaceBitmap (TOOLBAR_INFO *infoPtr, const TBREPLACEBITMAP *lpReplace)
{
    HBITMAP hBitmap;
    ...
    if (!res)
    {
        ...
        if (!TOOLBAR_SendNotify(&nmtbr.hdr, infoPtr, TBN_RESTORE))
        {
            ...
            for (i = 0; i < count; i++)
            {
                ...
                _InsertButtonT(infoPtr, -1, &nmtbr.tbButton, TRUE);
            }
            ...
            for (i = 0; ; i++)
            {
                ...
                _SetButtonInfo( infoPtr, tb.tbButton.idCommand, &bi, TRUE );
            }
...
static LRESULT _SaveRestoreW (TOOLBAR_INFO *infoPtr, WPARAM wParam, const TBSAVEPARAMSW *lpSave)
{
...
static LRESULT _SaveRestoreA (TOOLBAR_INFO *infoPtr, WPARAM wParam, const TBSAVEPARAMSA *lpSave)
{
    ...
    pszSubKey = (WCHAR*)Alloc(len * sizeof(WCHAR));
    ...
    pszValueName = (WCHAR*)Alloc(len * sizeof(WCHAR));
    ...
    result = _SaveRestoreW(infoPtr, wParam, &SaveW);
...
static LRESULT _SetAnchorHighlight (TOOLBAR_INFO *infoPtr, BOOL bAnchor)
{
...
static LRESULT _SetBitmapSize (TOOLBAR_INFO *infoPtr, WPARAM wParam, LPARAM lParam)
{
...
static LRESULT _SetButtonSize (TOOLBAR_INFO *infoPtr, LPARAM lParam)
{
...
static LRESULT _SetButtonWidth (TOOLBAR_INFO *infoPtr, LPARAM lParam)
{
...
static LRESULT _SetCmdId (TOOLBAR_INFO *infoPtr, INT nIndex, INT nId)
{
    if ((nIndex < 0) || (nIndex >= infoPtr->nNumButtons))
	return FALSE;
...
static LRESULT _SetDisabledImageList (TOOLBAR_INFO *infoPtr, WPARAM wParam, HIMAGELIST himl)
{
    HIMAGELIST himlTemp;
    INT id = 0;
...
static LRESULT _SetDrawTextFlags (TOOLBAR_INFO *infoPtr, DWORD mask, DWORD flags)
{
    DWORD old_flags;
 ...
 */
static LRESULT _SetExtendedStyle (TOOLBAR_INFO *infoPtr, DWORD mask, DWORD style)
{
...
static LRESULT _SetHotImageList (TOOLBAR_INFO *infoPtr, WPARAM wParam, HIMAGELIST himl)
{
    HIMAGELIST himlTemp;
    INT id = 0;
...
static LRESULT _SetHotItem (TOOLBAR_INFO *infoPtr, INT nHotItem)
{
...
static LRESULT _SetImageList (TOOLBAR_INFO *infoPtr, WPARAM wParam, HIMAGELIST himl)
{
    HIMAGELIST himlTemp;
    ...
    if ((oldBitmapWidth != infoPtr->nBitmapWidth) || (oldBitmapHeight != infoPtr->nBitmapHeight))
    {
        ...
        if (infoPtr->nButtonWidth < oldButtonWidth)
            _SetButtonSize(infoPtr, MAKELONG(oldButtonWidth, infoPtr->nButtonHeight));
    }
...
static LRESULT _SetIndent (TOOLBAR_INFO *infoPtr, INT nIndent)
{
...
static LRESULT _SetInsertMark (TOOLBAR_INFO *infoPtr, const TBINSERTMARK *lptbim)
{
...
static LRESULT _SetInsertMarkColor (TOOLBAR_INFO *infoPtr, COLORREF clr)
{
...
static LRESULT _SetMaxTextRows (TOOLBAR_INFO *infoPtr, INT nMaxRows)
{
 ...
 */
static LRESULT _SetPadding (TOOLBAR_INFO *infoPtr, LPARAM lParam)
{
    DWORD  oldPad;
...
static LRESULT _SetParent (TOOLBAR_INFO *infoPtr, HWND hParent)
{
    HWND hwndOldNotify;
...
static LRESULT _SetRows (TOOLBAR_INFO *infoPtr, WPARAM wParam, LPRECT lprc)
{
...
static LRESULT _SetState (TOOLBAR_INFO *infoPtr, INT Id, LPARAM lParam)
{
    TBUTTON_INFO *btnPtr;
    INT nIndex;
...
static LRESULT _SetStyle (TOOLBAR_INFO *infoPtr, DWORD style)
{
...
static inline LRESULT _SetToolTips (TOOLBAR_INFO *infoPtr, HWND hwndTooltip)
{
...
static LRESULT _SetUnicodeFormat (TOOLBAR_INFO *infoPtr, WPARAM wParam)
{
    BOOL bTemp;
...
static LRESULT _GetColorScheme (const TOOLBAR_INFO *infoPtr, LPCOLORSCHEME lParam)
{
...
static LRESULT _SetColorScheme (TOOLBAR_INFO *infoPtr, const COLORSCHEME *lParam)
{
...
static LRESULT _SetVersion (TOOLBAR_INFO *infoPtr, INT iVersion)
{
    ...
    if (infoPtr->iVersion >= 5)
        _SetUnicodeFormat(infoPtr, TRUE);

    return iOldVersion;
}

static LRESULT _GetStringA (const TOOLBAR_INFO *infoPtr, WPARAM wParam, LPSTR str)
{
...
static LRESULT _GetStringW (const TOOLBAR_INFO *infoPtr, WPARAM wParam, LPWSTR str)
{
    WORD iString = HIWORD(wParam);
...
static LRESULT _SetBoundingSize(HWND hwnd, WPARAM wParam, LPARAM lParam)
{
...
static LRESULT _SetHotItem2 (TOOLBAR_INFO *infoPtr, INT nHotItem, LPARAM lParam)
{
    INT nOldHotItem = infoPtr->nHotItem;
 ...
 * toolbars. */
static LRESULT _SetListGap(TOOLBAR_INFO *infoPtr, INT iListGap)
{
 ...
 * various states. */
static LRESULT _GetImageListCount(const TOOLBAR_INFO *infoPtr)
{
    TRACE("hwnd=%p\n", infoPtr->hwndSelf);
...
static LRESULT _GetIdealSize (const TOOLBAR_INFO *infoPtr, WPARAM wParam, LPARAM lParam)
{
...
static LRESULT _Unkwn464(HWND hwnd, WPARAM wParam, LPARAM lParam)
{
...
static LRESULT _Create (HWND hwnd, const CREATESTRUCTW *lpcs)
{
    ...
    //fixme infoPtr->hTheme = OpenThemeDataForDpi (NULL, themeClass, GetDpiForWindow (hwnd));
...
static LRESULT _Destroy (TOOLBAR_INFO *infoPtr)
{
    INT i;
...
static LRESULT _EraseBackground (TOOLBAR_INFO *infoPtr, WPARAM wParam, LPARAM lParam)
{
    NMTBCUSTOMDRAW tbcd;
    INT ret = FALSE;
    DWORD ntfret;
...
static inline LRESULT _GetFont (const TOOLBAR_INFO *infoPtr)
{
...
static void TOOLBAR_SetRelativeHotItem(TOOLBAR_INFO *infoPtr, INT iDirection, DWORD dwReason)
{
    ...
    {
        /* did we wrap? */
        if ((nNewHotItem + iDirection < 0) || (nNewHotItem + iDirection >= infoPtr->nNumButtons))
        {
...
static LRESULT _KeyDown (TOOLBAR_INFO *infoPtr, WPARAM wParam, LPARAM lParam)
{
    NMKEY nmkey;
...
static LRESULT _LButtonDblClk (TOOLBAR_INFO *infoPtr, WPARAM wParam, LPARAM lParam)
{
    POINT pt;
    BOOL button;
    ...
    if (button)
        _LButtonDown (infoPtr, wParam, lParam);
...
static LRESULT _LButtonUp (TOOLBAR_INFO *infoPtr, WPARAM wParam, LPARAM lParam)
{
    TBUTTON_INFO *btnPtr;
    POINT pt;
    INT   nHit;
            ...
            if (nButton == infoPtr->nButtonDrag)
            {
                ...
                if (pt.x < (btnPtr->rect.left + (btnPtr->rect.right - btnPtr->rect.left)/2))
                {
                    if ((nButton > 0) && (infoPtr->buttons[nButton-1].fsStyle & BTNS_SEP))
                        _DeleteButton(infoPtr, nButton - 1);
                }
                else /* else insert a separator before the dragged button */
                {
                    ...
                    tbb.iString = -1;
                    _InsertButtonT(infoPtr, nButton, &tbb, TRUE);
                }
            }
            else
            {
                if (nButton == -1)
                {
                    if ((infoPtr->nNumButtons > 0) && (pt.x < infoPtr->buttons[0].rect.left))
                        _MoveButton(infoPtr, infoPtr->nButtonDrag, 0);
                    else
                        _MoveButton(infoPtr, infoPtr->nButtonDrag, infoPtr->nNumButtons);
                }
                else
                    _MoveButton(infoPtr, infoPtr->nButtonDrag, nButton);
            }
        }
        else
        {
            TRACE("button %d dragged out of toolbar\n", infoPtr->nButtonDrag);
            _DeleteButton(infoPtr, infoPtr->nButtonDrag);
        }
...
static LRESULT _RButtonUp(TOOLBAR_INFO *infoPtr, WPARAM wParam, LPARAM lParam)
{
    INT nHit;
    NMMOUSE nmmouse;
    POINT pt;
...
static LRESULT _RButtonDblClk( TOOLBAR_INFO *infoPtr, WPARAM wParam, LPARAM lParam)
{
    NMHDR nmhdr;
...
static LRESULT _CaptureChanged(TOOLBAR_INFO *infoPtr)
{
    TBUTTON_INFO *btnPtr;
...
static LRESULT _MouseLeave (TOOLBAR_INFO *infoPtr)
{
...
static LRESULT _MouseMove (TOOLBAR_INFO *infoPtr, WPARAM wParam, LPARAM lParam)
{
    POINT pt;
    ...
    if (infoPtr->hwndToolTip)
	    TOOLBAR_RelayEvent (infoPtr->hwndToolTip, infoPtr->hwndSelf, WM_MOUSEMOVE, wParam, lParam);
...
static inline LRESULT _NCActivate (HWND hwnd, WPARAM wParam, LPARAM lParam)
{
/*    if (wndPtr->dwStyle & CCS_NODIVIDER) */
	return DefWindowProcW (hwnd, WM_NCACTIVATE, wParam, lParam);
/*    else */
/*	return _NCPaint (wndPtr, wParam, lParam); */
}

static inline LRESULT _NCCalcSize (HWND hwnd, WPARAM wParam, LPARAM lParam)
{
...
static LRESULT _NCCreate (HWND hwnd, WPARAM wParam, const CREATESTRUCTW *lpcs)
{
    TOOLBAR_INFO *infoPtr;
    DWORD styleadd = 0;

    /* allocate memory for info structure */
    infoPtr = new TOOLBAR_INFO{0};//(TOOLBAR_INFO*)Alloc (sizeof(TOOLBAR_INFO));
...
static LRESULT _NCPaint (HWND hwnd, WPARAM wParam, LPARAM lParam)
{
...
static LRESULT TOOLBAR_TTGetDispInfo (TOOLBAR_INFO *infoPtr, NMTTDISPINFOW *lpnmtdi)
{
    ...
    if (infoPtr->bUnicode)
    {
        ...
        if (len > ARRAY_SIZE(lpnmtdi->szText) - 1)
        {
             ...
             * tooltip control */
            infoPtr->pszTooltipText = (WCHAR*)Alloc((len+1)*sizeof(WCHAR));
    ...
    else
    {
        ...
        if (len > ARRAY_SIZE(lpnmtdi->szText))
        {
             ...
             * isn't enough space in buffer passed to us by the
             * tooltip control */
            infoPtr->pszTooltipText = (WCHAR*)Alloc(len*sizeof(WCHAR));
            if (infoPtr->pszTooltipText)
            {
    ...
    */
    ...
    {
        MultiByteToWideChar(CP_ACP, 0, nmtdi.lpszText, -1, lpnmtdi->lpszText, strlen(nmtdi.szText));
        return 0;
    }
...
static inline LRESULT _Notify (TOOLBAR_INFO *infoPtr, LPNMHDR lpnmh)
{
    switch (lpnmh->code)
    {
    case PGN_CALCSIZE:
...
static LRESULT _NotifyFormat(const TOOLBAR_INFO *infoPtr, WPARAM wParam, LPARAM lParam)
{
    LRESULT format;
    ...
    if (lParam == NF_REQUERY) {
	    format = SendMessageW(infoPtr->hwndNotify, WM_NOTIFYFORMAT, (WPARAM)infoPtr->hwndSelf, NF_QUERY);
...
static LRESULT _Paint (TOOLBAR_INFO *infoPtr, WPARAM wParam)
{
    HDC hdc;
    PAINTSTRUCT ps;
...
static LRESULT _SetFocus (TOOLBAR_INFO *infoPtr)
{
    TRACE("nHotItem = %d\n", infoPtr->nHotItem);
...
static LRESULT _SetFont(TOOLBAR_INFO *infoPtr, HFONT hFont, WORD Redraw)
{
...
static LRESULT _SetRedraw (TOOLBAR_INFO *infoPtr, WPARAM wParam)
/*****************************************************
      *
      * Function;
...
static LRESULT _Size (TOOLBAR_INFO *infoPtr)
{
...
static LRESULT _StyleChanged (TOOLBAR_INFO *infoPtr, INT nType, const STYLESTRUCT *lpStyle)
{
    if (nType == GWL_STYLE)
        return _SetStyle(infoPtr, lpStyle->styleNew);

    return 0;
}

static LRESULT _SysColorChange (void)
{
...
{
    CloseThemeData (infoPtr->hTheme);
    //fixme infoPtr->hTheme = OpenThemeDataForDpi (NULL, themeClass, GetDpiForWindow (infoPtr->hwndSelf));
    infoPtr->hTheme = OpenThemeData (NULL, themeClass);
    InvalidateRect (infoPtr->hwndSelf, NULL, TRUE);
    return 0;
}
...
static LRESULT WINAPI ToolbarWindowProc (HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (uMsg)
    {
	case TB_ADDBITMAP: return _AddBitmap (infoPtr, (INT)wParam, (TBADDBITMAP*)lParam);
	case TB_ADDBUTTONSA:
	case TB_ADDBUTTONSW: return _AddButtonsT (infoPtr, wParam, (LPTBBUTTON)lParam, uMsg == TB_ADDBUTTONSW);
	case TB_ADDSTRINGA: return _AddStringA (infoPtr, (HINSTANCE)wParam, lParam);
	case TB_ADDSTRINGW: return _AddStringW (infoPtr, (HINSTANCE)wParam, lParam);
	case TB_AUTOSIZE: return _AutoSize (infoPtr);
	case TB_BUTTONCOUNT: return _ButtonCount (infoPtr);
	case TB_BUTTONSTRUCTSIZE: return _ButtonStructSize (infoPtr, wParam);
	case TB_CHANGEBITMAP: return _ChangeBitmap (infoPtr, wParam, LOWORD(lParam));
	case TB_CHECKBUTTON: return _CheckButton (infoPtr, wParam, lParam);
	case TB_COMMANDTOINDEX: return _CommandToIndex (infoPtr, wParam);
	case TB_CUSTOMIZE: return _Customize (infoPtr);
	case TB_DELETEBUTTON: return _DeleteButton (infoPtr, wParam);
	case TB_ENABLEBUTTON: return _EnableButton (infoPtr, wParam, lParam);
	case TB_GETANCHORHIGHLIGHT: return _GetAnchorHighlight (infoPtr);
	case TB_GETBITMAP: return _GetBitmap (infoPtr, wParam);
	case TB_GETBITMAPFLAGS: return _GetBitmapFlags ();
	case TB_GETBUTTON: return _GetButton (infoPtr, wParam, (TBBUTTON*)lParam);
	case TB_GETBUTTONINFOA:
	case TB_GETBUTTONINFOW: return _GetButtonInfoT (infoPtr, wParam, (LPTBBUTTONINFOW)lParam, uMsg == TB_GETBUTTONINFOW);
	case TB_GETBUTTONSIZE: return _GetButtonSize (infoPtr);
	case TB_GETBUTTONTEXTA:
	case TB_GETBUTTONTEXTW: return _GetButtonText (infoPtr, wParam, (LPWSTR)lParam, uMsg == TB_GETBUTTONTEXTW);
	case TB_GETDISABLEDIMAGELIST: return _GetDisabledImageList (infoPtr, wParam);
	case TB_GETEXTENDEDSTYLE: return _GetExtendedStyle (infoPtr);
	case TB_GETHOTIMAGELIST: return _GetHotImageList (infoPtr, wParam);
	case TB_GETHOTITEM: return _GetHotItem (infoPtr);
	case TB_GETIMAGELIST: return _GetDefImageList (infoPtr, wParam);
	case TB_GETINSERTMARK: return _GetInsertMark (infoPtr, (TBINSERTMARK*)lParam);
	case TB_GETINSERTMARKCOLOR: return _GetInsertMarkColor (infoPtr);
	case TB_GETITEMRECT: return _GetItemRect (infoPtr, wParam, (LPRECT)lParam);
	case TB_GETMAXSIZE: return _GetMaxSize (infoPtr, (LPSIZE)lParam);
/*	case TB_GETOBJECT:			*/ /* 4.71 */
	case TB_GETPADDING: return _GetPadding (infoPtr);
	case TB_GETRECT: return _GetRect (infoPtr, wParam, (LPRECT)lParam);
	case TB_GETROWS: return _GetRows (infoPtr);
	case TB_GETSTATE: return _GetState (infoPtr, wParam);
	case TB_GETSTRINGA: return _GetStringA (infoPtr, wParam, (LPSTR)lParam);
	case TB_GETSTRINGW: return _GetStringW (infoPtr, wParam, (LPWSTR)lParam);
	case TB_GETSTYLE: return _GetStyle (infoPtr);
	case TB_GETTEXTROWS: return _GetTextRows (infoPtr);
	case TB_GETTOOLTIPS: return _GetToolTips (infoPtr);
	case TB_GETUNICODEFORMAT: return _GetUnicodeFormat (infoPtr);
	case TB_HIDEBUTTON: return _HideButton (infoPtr, wParam, LOWORD(lParam));
	case TB_HITTEST: return _HitTest (infoPtr, (LPPOINT)lParam);
	case TB_INDETERMINATE: return _Indeterminate (infoPtr, wParam, LOWORD(lParam));
	case TB_INSERTBUTTONA:
	case TB_INSERTBUTTONW: return _InsertButtonT(infoPtr, wParam, (TBBUTTON*)lParam, uMsg == TB_INSERTBUTTONW);
/*	case TB_INSERTMARKHITTEST:		*/ /* 4.71 */
	case TB_ISBUTTONCHECKED: return _IsButtonChecked (infoPtr, wParam);
	case TB_ISBUTTONENABLED: return _IsButtonEnabled (infoPtr, wParam);
	case TB_ISBUTTONHIDDEN:  return _IsButtonHidden (infoPtr, wParam);
	case TB_ISBUTTONHIGHLIGHTED: return _IsButtonHighlighted (infoPtr, wParam);
	case TB_ISBUTTONINDETERMINATE:  return _IsButtonIndeterminate (infoPtr, wParam);
	case TB_ISBUTTONPRESSED: return _IsButtonPressed (infoPtr, wParam);
    case TB_LOADIMAGES: return _LoadImages (infoPtr, wParam, (HINSTANCE)lParam);
	case TB_MAPACCELERATORA:
	case TB_MAPACCELERATORW: return _MapAccelerator (infoPtr, wParam, (UINT*)lParam);
    case TB_MARKBUTTON: return _MarkButton (infoPtr, wParam, LOWORD(lParam));
	case TB_MOVEBUTTON: return _MoveButton (infoPtr, wParam, lParam);
	case TB_PRESSBUTTON: return _PressButton (infoPtr, wParam, LOWORD(lParam));
	case TB_REPLACEBITMAP: return _ReplaceBitmap (infoPtr, (LPTBREPLACEBITMAP)lParam);
	case TB_SAVERESTOREA: return _SaveRestoreA (infoPtr, wParam, (LPTBSAVEPARAMSA)lParam);
	case TB_SAVERESTOREW: return _SaveRestoreW (infoPtr, wParam, (LPTBSAVEPARAMW)lParam);
	case TB_SETANCHORHIGHLIGHT: return _SetAnchorHighlight (infoPtr, (BOOL)wParam);
	case TB_SETBITMAPSIZE: return _SetBitmapSize (infoPtr, wParam, lParam);
	case TB_SETBUTTONINFOA:
	case TB_SETBUTTONINFOW: return _SetButtonInfo (infoPtr, wParam, (LPTBBUTTONINFOW)lParam, uMsg == TB_SETBUTTONINFOW);
	case TB_SETBUTTONSIZE: return _SetButtonSize (infoPtr, lParam);
	case TB_SETBUTTONWIDTH:  return _SetButtonWidth (infoPtr, lParam);
	case TB_SETCMDID: return _SetCmdId (infoPtr, wParam, lParam);
	case TB_SETDISABLEDIMAGELIST: return _SetDisabledImageList (infoPtr, wParam, (HIMAGELIST)lParam);
	case TB_SETDRAWTEXTFLAGS: return _SetDrawTextFlags (infoPtr, wParam, lParam);
	case TB_SETEXTENDEDSTYLE: return _SetExtendedStyle (infoPtr, wParam, lParam);
	case TB_SETHOTIMAGELIST: return _SetHotImageList (infoPtr, wParam, (HIMAGELIST)lParam);
	case TB_SETHOTITEM: return _SetHotItem (infoPtr, wParam);
	case TB_SETIMAGELIST: return _SetImageList (infoPtr, wParam, (HIMAGELIST)lParam);
	case TB_SETINDENT: return _SetIndent (infoPtr, wParam);
	case TB_SETINSERTMARK: return _SetInsertMark (infoPtr, (TBINSERTMARK*)lParam);
	case TB_SETINSERTMARKCOLOR: return _SetInsertMarkColor (infoPtr, lParam);
	case TB_SETMAXTEXTROWS: return _SetMaxTextRows (infoPtr, wParam);
	case TB_SETPADDING: return _SetPadding (infoPtr, lParam);
	case TB_SETPARENT: return _SetParent (infoPtr, (HWND)wParam);
	case TB_SETROWS: return _SetRows (infoPtr, wParam, (LPRECT)lParam);
	case TB_SETSTATE:  return _SetState (infoPtr, wParam, lParam);
	case TB_SETSTYLE:  return _SetStyle (infoPtr, lParam);
	case TB_SETTOOLTIPS: return _SetToolTips (infoPtr, (HWND)wParam);
	case TB_SETUNICODEFORMAT: return _SetUnicodeFormat (infoPtr, wParam);
	case TB_SETBOUNDINGSIZE: return _SetBoundingSize(hwnd, wParam, lParam);
	case TB_SETHOTITEM2: return _SetHotItem2 (infoPtr, wParam, lParam);
	case TB_SETLISTGAP: return _SetListGap(infoPtr, wParam);
	case TB_GETIMAGELISTCOUNT: return _GetImageListCount(infoPtr);
	case TB_GETIDEALSIZE: return _GetIdealSize (infoPtr, wParam, lParam);
	case TB_UNKWN464: return _Unkwn464(hwnd, wParam, lParam);
    /* Common Control Messages */
    /*	case TB_GETCOLORSCHEME:			*/ /* identical to CCM_ */
	case CCM_GETCOLORSCHEME: return _GetColorScheme (infoPtr, (LPCOLORSCHEME)lParam);
    /*	case TB_SETCOLORSCHEME:			*/ /* identical to CCM_ */
	case CCM_SETCOLORSCHEME: return _SetColorScheme (infoPtr, (LPCOLORSCHEME)lParam);
	case CCM_GETVERSION: return _GetVersion (infoPtr);
	case CCM_SETVERSION: return _SetVersion (infoPtr, (INT)wParam);
    /*	case WM_CHAR: */
	case WM_CREATE: return _Create (hwnd, (CREATESTRUCTW*)lParam);
	case WM_DESTROY: return _Destroy (infoPtr);
	case WM_ERASEBKGND: return _EraseBackground (infoPtr, wParam, lParam);
	case WM_GETFONT: return _GetFont (infoPtr);
	case WM_KEYDOWN: return _KeyDown (infoPtr, wParam, lParam);
    /*	case WM_KILLFOCUS: */
	case WM_LBUTTONDBLCLK: return _LButtonDblClk (infoPtr, wParam, lParam);
	case WM_LBUTTONDOWN: return _LButtonDown (infoPtr, wParam, lParam);
	case WM_LBUTTONUP: return _LButtonUp (infoPtr, wParam, lParam);
	case WM_RBUTTONUP: return _RButtonUp (infoPtr, wParam, lParam);
	case WM_RBUTTONDBLCLK: return _RButtonDblClk (infoPtr, wParam, lParam);
	case WM_MOUSEMOVE: return _MouseMove (infoPtr, wParam, lParam);
	case WM_MOUSELEAVE: return _MouseLeave (infoPtr);
	case WM_CAPTURECHANGED: if (hwnd == (HWND)lParam) return 0; return _CaptureChanged(infoPtr);
	case WM_NCACTIVATE: return _NCActivate (hwnd, wParam, lParam);
	case WM_NCCALCSIZE:  return _NCCalcSize (hwnd, wParam, lParam);
	case WM_NCCREATE: return _NCCreate (hwnd, wParam, (CREATESTRUCTW*)lParam);
	case WM_NCPAINT: return _NCPaint (hwnd, wParam, lParam);
	case WM_NOTIFY: return _Notify (infoPtr, (LPNMHDR)lParam);
	case WM_NOTIFYFORMAT: return _NotifyFormat (infoPtr, wParam, lParam);
	case WM_PRINTCLIENT: 
    case WM_PAINT:  return _Paint (infoPtr, wParam);
	case WM_SETFOCUS: return _SetFocus (infoPtr);
	case WM_SETFONT: return _SetFont(infoPtr, (HFONT)wParam, (WORD)lParam);
	case WM_SETREDRAW: return _SetRedraw (infoPtr, wParam);
	case WM_SIZE: return _Size (infoPtr);
	case WM_STYLECHANGED: return _StyleChanged (infoPtr, (INT)wParam, (LPSTYLESTRUCT)lParam);
	case WM_SYSCOLORCHANGE: return _SysColorChange ();
	...
	case WM_DRAWITEM:
	case WM_MEASUREITEM:
	case WM_VKEYTOITEM: return SendMessageW (infoPtr->hwndNotify, uMsg, wParam, lParam);
	/* We see this in Outlook Express 5.x and just does DefWindowProc */
    case PGM_FORWARDMOUSE: return DefWindowProcW (hwnd, uMsg, wParam, lParam);
	default:
...
static HIMAGELIST TOOLBAR_InsertImageList(PIMLENTRY **pies, INT *cies, HIMAGELIST himl, INT id)
{
    ...
    if (!c)
    {
        ...
        c = (IMLENTRY*)Alloc(sizeof(IMLENTRY));
