...
static inline int notify_customdraw (const TRACKBAR_INFO *infoPtr, NMCUSTOMDRAW *pnmcd, int stage)
{
...
static inline LONG TRACKBAR_ConvertPlaceToPosition (const TRACKBAR_INFO *infoPtr, int place)
{
...
static inline void TRACKBAR_PageDown (TRACKBAR_INFO *infoPtr)
{
...
static inline void TRACKBAR_PageUp (TRACKBAR_INFO *infoPtr)
{
...
static inline void TRACKBAR_UpdateThumb (TRACKBAR_INFO *infoPtr)
{
	TRACKBAR_CalcThumb(infoPtr, infoPtr->lPos, &infoPtr->rcThumb);
}

static inline void TRACKBAR_InvalidateAll (const TRACKBAR_INFO *infoPtr)
{
...
static inline void TRACKBAR_InvalidateThumbMove (const TRACKBAR_INFO *infoPtr, LONG oldPos, LONG newPos)
{
...
static inline BOOL TRACKBAR_HasSelection (const TRACKBAR_INFO *infoPtr)
{
...
static void TRACKBAR_DrawChannel (const TRACKBAR_INFO *infoPtr, HDC hdc)
{
	...
	else
	{
		...
		if (infoPtr->dwStyle & TBS_ENABLESELRANGE) {		 /* fill the channel */
			FillRect (hdc, &rcChannel, (HBRUSH)GetStockObject(WHITE_BRUSH));
...
static inline void TRACKBAR_DrawTic (const TRACKBAR_INFO *infoPtr, HDC hdc, LONG ticPos, int flags)
{
...
static void TRACKBAR_DrawTics (const TRACKBAR_INFO *infoPtr, HDC hdc)
{
	...
	hOldPen = (HPEN)(hTicPen ? SelectObject(hdc, hTicPen) : 0);
...
static int TRACKBAR_FillThumb (const TRACKBAR_INFO *infoPtr, HDC hdc, HBRUSH hbrush)
{
	...
	oldbr = (HBRUSH)SelectObject(hdc, hbrush);
	SetPolyFillMode(hdc, WINDING);
	// todo! ARRAY_SIZE(points)
...
static inline void TRACKBAR_ActivateToolTip (const TRACKBAR_INFO *infoPtr, BOOL fShow)
{
	TTTOOLINFOW ti;
...
static void TRACKBAR_Refresh (TRACKBAR_INFO *infoPtr, HDC hdcDst)
{
	...
	if (hdc) {
		...
		if (hOffScreenBmp) {
			hOldBmp = (HBITMAP)SelectObject(hdc, hOffScreenBmp);
		} else {
			DeleteObject(hdc);
			hdc = hdcDst;
		}
...
static LRESULT _ClearSel (TRACKBAR_INFO *infoPtr, BOOL fRedraw)
{
	infoPtr->lSelMin = 0;
	infoPtr->lSelMax = 0;
...
static inline LRESULT _GetChannelRect (const TRACKBAR_INFO *infoPtr, LPRECT lprc)
{
...
static inline LONG _GetNumTics (const TRACKBAR_INFO *infoPtr)
{
...
static inline LONG _GetTicPos (const TRACKBAR_INFO *infoPtr, INT iTic)
{
...
static HWND _SetBuddy (TRACKBAR_INFO *infoPtr, BOOL fLocation, HWND hwndBuddy)
{
	HWND hwndTemp;
...
static inline LONG _SetLineSize (TRACKBAR_INFO *infoPtr, LONG lLineSize)
{
...
static inline LONG _SetPageSize (TRACKBAR_INFO *infoPtr, LONG lPageSize)
{
...
static inline LRESULT _SetRange (TRACKBAR_INFO *infoPtr, BOOL redraw, LONG range)
{
...
static inline LRESULT _SetRangeMin (TRACKBAR_INFO *infoPtr, BOOL redraw, LONG lMin)
{
...
static inline LRESULT _SetSel (TRACKBAR_INFO *infoPtr, BOOL fRedraw, LONG lSel)
{
	if (!(infoPtr->dwStyle & TBS_ENABLESELRANGE)){
		infoPtr->lSelMin = 0;
		infoPtr->lSelMax = 0;
...
static inline LRESULT _SetSelEnd (TRACKBAR_INFO *infoPtr, BOOL fRedraw, LONG lEnd)
{
	if (!(infoPtr->dwStyle & TBS_ENABLESELRANGE)){
		infoPtr->lSelMax = 0;
		return 0;
...
static inline LRESULT _SetSelStart (TRACKBAR_INFO *infoPtr, BOOL fRedraw, LONG lStart)
{
	if (!(infoPtr->dwStyle & TBS_ENABLESELRANGE)){
		infoPtr->lSelMin = 0;
		return 0;
...
static inline LRESULT _SetThumbLength (TRACKBAR_INFO *infoPtr, UINT iLength)
{
...
static inline LRESULT _SetTic (TRACKBAR_INFO *infoPtr, LONG lPos)
{
...
static inline LRESULT _SetTicFreq (TRACKBAR_INFO *infoPtr, WORD wFreq)
{
	if (infoPtr->dwStyle & TBS_AUTOTICKS) {
...
static inline INT _SetTipSide (TRACKBAR_INFO *infoPtr, INT fLocation)
{
...
static inline LRESULT _SetToolTips (TRACKBAR_INFO *infoPtr, HWND hwndTT)
{
...
static inline BOOL _SetUnicodeFormat (TRACKBAR_INFO *infoPtr, BOOL fUnicode)
{
...
static LRESULT _InitializeThumb (TRACKBAR_INFO *infoPtr)
{
	int client_size;
	RECT rect;
...
static LRESULT _Destroy (TRACKBAR_INFO *infoPtr)
{
...
static LRESULT _KillFocus (TRACKBAR_INFO *infoPtr)
{
	TRACE("\n");
...
static LRESULT _LButtonDown (TRACKBAR_INFO *infoPtr, INT x, INT y)
{
	POINT clickPoint;
	...
	if ((infoPtr->dwStyle & TBS_FOCUSNEVER)==0)
	{
...
static LRESULT _LButtonUp (TRACKBAR_INFO *infoPtr)
{
	if (infoPtr->flags & TB_DRAG_MODE) {
...
static LRESULT _CaptureChanged (const TRACKBAR_INFO *infoPtr)
{
	notify_with_scroll (infoPtr, TB_ENDTRACK);
	return 0;
}

static LRESULT _Paint (TRACKBAR_INFO *infoPtr, HDC hdc)
{
	if (hdc) {
...
static LRESULT _SetFocus (TRACKBAR_INFO *infoPtr)
{
	TRACE("\n");
	if (((infoPtr->dwStyle & TBS_FOCUSNEVER)==0))
	{
...
static LRESULT _Size (TRACKBAR_INFO *infoPtr)
{
	if (infoPtr->dwStyle & TBS_FIXEDLENGTH)
	{
		TRACKBAR_CalcChannel(infoPtr);
...
static LRESULT _StyleChanged (TRACKBAR_INFO *infoPtr, WPARAM wStyleType, const STYLESTRUCT *lpss)
{
...
static LRESULT _Timer (TRACKBAR_INFO *infoPtr)
{
	if (infoPtr->flags & TB_AUTO_PAGE) {
		POINT pt;
		if (GetCursorPos(&pt))
...
static BOOL _KeyDown (TRACKBAR_INFO *infoPtr, INT nVirtKey)
{
...
static inline BOOL _KeyUp (const TRACKBAR_INFO *infoPtr, INT nVirtKey)
{
	switch (nVirtKey) {
	case VK_LEFT:
	case VK_UP:
...
static LRESULT _Enable (TRACKBAR_INFO *infoPtr, BOOL enable)
{
	if (enable)
...
static LRESULT WINAPI TRACKBAR_WindowProc (HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	switch (uMsg)
	{
	case TBM_CLEARSEL: return _ClearSel (infoPtr, (BOOL)wParam);
	case TBM_CLEARTICS: return _ClearTics (infoPtr, (BOOL)wParam);
	
	case TBM_GETBUDDY: return (LRESULT)(wParam?infoPtr->hwndBuddyLA:infoPtr->hwndBuddyRB);
	case TBM_GETCHANNELRECT: return _GetChannelRect (infoPtr, (LPRECT)lParam);
	case TBM_GETLINESIZE: return infoPtr->lLineSize;
	...
	case TBM_GETSELSTART: return infoPtr->lSelMin;
	case TBM_GETTHUMBLENGTH: return infoPtr->uThumbLen;
	case TBM_GETTHUMBRECT: return CopyRect((LPRECT)lParam, &infoPtr->rcThumb);
	case TBM_GETTIC: return _GetTic (infoPtr, (INT)wParam);
	case TBM_GETTICPOS: return _GetTicPos (infoPtr, (INT)wParam);
	case TBM_GETTOOLTIPS: return (LRESULT)infoPtr->hwndToolTip;
	case TBM_GETUNICODEFORMAT: return infoPtr->bUnicode;
	...
	case TBM_SETSEL: return _SetSel (infoPtr, (BOOL)wParam, (LONG)lParam);
	case TBM_SETSELEND: return _SetSelEnd (infoPtr, (BOOL)wParam, (LONG)lParam);
	case TBM_SETSELSTART: return _SetSelStart (infoPtr, (BOOL)wParam, (LONG)lParam);
	
	case TBM_SETTHUMBLENGTH: return _SetThumbLength (infoPtr, (UINT)wParam);
	case TBM_SETTIC: return _SetTic (infoPtr, (LONG)lParam);
	case TBM_SETTICFREQ: return _SetTicFreq (infoPtr, (WORD)wParam);
	case TBM_SETTIPSIDE: return _SetTipSide (infoPtr, (INT)wParam);
	case TBM_SETTOOLTIPS: return _SetToolTips (infoPtr, (HWND)wParam);
	case TBM_SETUNICODEFORMAT: return _SetUnicodeFormat (infoPtr, (BOOL)wParam);

	case WM_CAPTURECHANGED: return hwnd==(HWND)lParam?0 : _CaptureChanged (infoPtr);
	case WM_CREATE: return _Create (hwnd, (LPCREATESTRUCTW)lParam);
	case WM_DESTROY: return _Destroy (infoPtr);
	case WM_ENABLE: return _Enable (infoPtr, (BOOL)wParam);
	case WM_ERASEBKGND:
		return 0;
	case WM_GETDLGCODE: return DLGC_WANTARROWS;
	case WM_KEYDOWN: return _KeyDown (infoPtr, (INT)wParam);
	case WM_KEYUP: return _KeyUp (infoPtr, (INT)wParam);
	case WM_KILLFOCUS: return _KillFocus (infoPtr);
	case WM_LBUTTONDOWN: return _LButtonDown (infoPtr, (SHORT)LOWORD(lParam), (SHORT)HIWORD(lParam));
	case WM_LBUTTONUP: return _LButtonUp (infoPtr);
	case WM_MOUSELEAVE:
		infoPtr->flags &= ~TB_THUMB_HOT; 
		InvalidateRect (infoPtr->hwndSelf, &infoPtr->rcThumb, FALSE);
		return 0;
	case WM_MOUSEMOVE: return _MouseMove (infoPtr, (SHORT)LOWORD(lParam), (SHORT)HIWORD(lParam));
	case WM_PRINTCLIENT:
	...
	case WM_TIMER: return _Timer (infoPtr);
	case WM_WININICHANGE: return _InitializeThumb (infoPtr);
	default:
...
void TRACKBAR_Register (void)
{
	...
	wndClass.lpszClassName = TRACKBAR_CLASSM;
